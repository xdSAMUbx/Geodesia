...
def calc_MGNSRGS (a,e_cuadrado,b,e_prim_cuad,alfa,beta,gamma,delta,epsilon):
    ...
    eta = math.sqrt(e_prim_cuad*((math.cos(math.radians(lat_pto)))**2))
    ...
    if lon_pto >= 72.57750791667 and lon_pto <= 75.57750791667:
        ...
        norte = (arc_merid_pto-arc_merid_or)+((t/2)*gran_normal*((math.radians(l))**2)*((math.cos(math.radians(lat_pto)))**2))+((t/24)*gran_normal*((math.radians(l))**4)*((math.cos(math.radians(lat_pto)))**4)*(5-(t**2)+9*(eta**2)+4*(eta**4)))+((t/720)*gran_normal*((math.radians(l))**6)*((math.cos(math.radians(lat_pto)))**6)*(61-58*(t**2)+t**4+270*(eta**2)-330*(t**2)*(eta**2)))+((t/40320)*gran_normal*((math.radians(l))**8)*((math.cos(math.radians(lat_pto)))**8)*(1385-3111*(t**2)+543*(t**4)-(t**6)))+1000000
        este = (gran_normal*(math.radians(l))*(math.cos(math.radians(lat_pto))))+(1/6*gran_normal*((math.radians(l))**3)*((math.cos(math.radians(lat_pto)))**3)*(1-t**2+eta**2))+(1/120*gran_normal*((math.radians(l))**5)*((math.cos(math.radians(lat_pto)))**5)*(5-18*(t**2)+t**4+14*(eta**2)-58*(t**2)*(eta**2)))+(1/5040*gran_normal*((math.radians(l))**7)*((math.cos(math.radians(lat_pto)))**7)*(61-479*(t**2)+179*(t**4)-t**6))+1000000
