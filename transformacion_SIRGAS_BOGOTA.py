...
def calc_MGNSRGS (a,e_cuadrado,e_prim_cuad,alfa,beta,gamma,delta,epsilon):
...
def calc_DATBOG (a,e_cuadrado):
    ...
    sen_pto = math.sin(math.radians(lat_pto))
    gran_normal = (a/math.sqrt((1-(e_cuadrado*((sen_pto)**2)))))
    tan_pto = math.tan(math.radians(lat_pto))
    sen_seg = math.sin(math.radians(1/3600))
    ro = (a*(1-e_cuadrado))/(1-(e_cuadrado*((sen_pto)**2)))**(3/2)
    cos_pto = math.cos(math.radians(lat_pto))
    ...
    if lon_pto <= -72.58091667 and lon_pto >= -75.58091667:
        ...
        delta_lambda =  (lon_pto-lon_or)*3600
        delta_fi = (lat_pto-lat_or)*3600
            
        #Coordenadas Norte y Este
        norte = ((delta_fi*ro*sen_seg))+(((delta_lambda**2)*(sen_seg**2)*gran_normal*sen_pto*cos_pto)/2)+(((delta_lambda**4)*(sen_seg**4)*gran_normal*sen_pto*(cos_pto**3)*(5-(tan_pto**2)))/24)+1000000.0
        este = ((delta_lambda*cos_pto*gran_normal*sen_seg))+(((delta_lambda**3)*(sen_seg**3)*gran_normal*(cos_pto**3)*((gran_normal/ro)*(tan_pto**2)))/6)+(((delta_lambda**5)*(sen_seg**5)*gran_normal*(cos_pto**5)*(5-(18*(tan_pto**2))+(tan_pto**4)))/120)+1000000       
        ...
        print(f"La coordenada Norte para Oeste Oeste - DATUM BOGOTA con origen gauss - Krüeger  es: {norte:.3f}")
        print(f"La coordenada Este para Oeste Oeste - DATUM BOGOTA con origen gauss - Krüeger es: {este:.3f}")
...
def const_ref ():
    ...
    print("¿Que elipsoide va a trabajar como referencia?")
    ...
    if elip == 1:
        ...
        calc_MGNSRGS (a,e_cuadrado,e_prim_cuad,alfa,beta,gamma,delta,epsilon)
    else:
        print("Error")
